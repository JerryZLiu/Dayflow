...
struct ActivityCard: View {
    ...
    var body: some View {
        if let activity = activity {
            ...
            .sheet(isPresented: $showSlideshowPlayer, onDismiss: {
                slideshowScreenshots = []
                slideshowTitle = nil
                slideshowStartTime = nil
                slideshowEndTime = nil
    ...
    private func openSlideshow(for activity: TimelineActivity, cardId: Int64) {
        ...
        AnalyticsService.shared.capture("timelapse_slideshow_started", [
            "card_id": cardId
        ])

        Task {
            do {
                let screenshots = try await ActivityCardTimelapseGenerator.shared.screenshots(forCardId: cardId)
                await MainActor.run {
                ...
                AnalyticsService.shared.capture("timelapse_slideshow_completed", [
                    "card_id": cardId,
                ...
                AnalyticsService.shared.capture("timelapse_slideshow_failed", [
                    "card_id": cardId,
    ...
    private func loadTimelapsePreviewThumbnail(for activity: TimelineActivity, size: CGSize) {
        ...
        Task {
            let screenshotURL = await ActivityCardTimelapseGenerator.shared.middleScreenshotURL(forCardId: cardId)
            await MainActor.run {
...
private actor ActivityCardTimelapseGenerator {
    ...
    func screenshots(forCardId cardId: Int64) throws -> [Screenshot] {
        ...
        let screenshots = storage.fetchScreenshotsInTimeRange(startTs: timelineCard.startTs, endTs: timelineCard.endTs)
...
private final class ScreenshotFilmstripGenerator {
    ...
    private static func cacheKey(for screenshots: [Screenshot], frameCount: Int, targetHeight: CGFloat) -> String {
        ...
        return "\(screenshots.count)|\(firstTs)|\(lastTs)|\(firstPath)|\(lastPath)|n:\(frameCount)|h:\(Int(targetHeight.rounded()))"
    }
...
private final class ScreenshotSlideshowPlaybackModel: ObservableObject {
    ...
    init(screenshots: [Screenshot], maxRenderHeight: Int) {
        ...
        self.loader = ScreenshotSlideshowFrameLoader(screenshots: screenshots, maxRenderHeight: maxRenderHeight)
        ...
        if screenshots.count > 1 {
            let totalSeconds = Double(max(1, screenshots.last!.capturedAt - screenshots.first!.capturedAt))
...
private final class ScreenshotSlideshowFrameLoader: @unchecked Sendable {
    ...
    private let decodeQueue = DispatchQueue(label: "com.dayflow.slideshow.decode", qos: .userInitiated)
